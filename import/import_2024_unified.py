...
def create_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS questoes (
            ...
            e TEXT,
            gabarito TEXT,
            fonte TEXT DEFAULT 'Processo Seletivo 2024',
...
def extract_questions_from_txt_2024(txt_path="2024_questions_alt.txt"):
    """Extrai questões da prova 2024 do arquivo .txt"""
    ...
    questions = extract_questions_from_text_2024(text)
    return questions

def extract_questions_from_text_2024(text):
    """Extrai questões do texto da prova 2024"""
...
def import_questions_with_images_2024():
    """Importa questões com imagens e alternativas para 2024"""
    print("IMPORTADOR UNIFICADO - PROVA 2024")
    print("=" * 50)
    ...
    questions = extract_questions_from_txt_2024()
    if not questions:
    ...
    image_files = get_image_files_2024()
    if not image_files:
    ...
    try:
        ...
        # Limpar questões existentes da fonte 2024
        cursor.execute("DELETE FROM questoes WHERE fonte = ?", ('Processo Seletivo 2024',))
        print("Removidas questões existentes da fonte: Processo Seletivo 2024")
        ...
        for filename in image_files:
            try:
                ...
                image_path = os.path.join("2024_questions_imgs", filename)
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    f"Questão {questao_num} - Processo Seletivo 2024",
                    question_data['a'],
                    question_data['b'],
                    question_data['c'],
                    question_data['d'],
...
def main():
    """Função principal para importar prova 2024 unificada"""
    success = import_questions_with_images_2024()
    
    if success:
        print("\n🎉 Importação da prova 2024 concluída com sucesso!")
