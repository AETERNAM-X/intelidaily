...
def create_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS questoes (
            ...
            e TEXT,
            gabarito TEXT,
            fonte TEXT DEFAULT 'Processo Seletivo 2025',
...
def extract_questions_from_txt_2025(txt_path="2025_questions_alt.txt"):
    """Extrai questões da prova 2025 do arquivo .txt"""
    ...
    questions = extract_questions_from_text_2025(text)
    return questions

def extract_questions_from_text_2025(text):
    """Extrai questões do texto da prova 2025"""
...
def get_image_files_2025():
    ...
    # Ordenar arquivos numericamente (questao_1.webp, questao_2.webp, etc.)
...
def import_questions_with_images_2025():
    """Importa questões com imagens e alternativas para 2025"""
    print("IMPORTADOR UNIFICADO - PROVA 2025")
    print("=" * 50)
    ...
    questions = extract_questions_from_txt_2025()
    if not questions:
        print("Nenhuma questão encontrada no arquivo de texto!")
        print("⚠️  Certifique-se de que o arquivo '2025_questions_alt.txt' existe e contém as questões")
        return False
    
    print(f"Extraídas {len(questions)} questões do arquivo de texto")
    
    # Obter arquivos de imagem
    image_files = get_image_files_2025()
    if not image_files:
    ...
    try:
        ...
        # Limpar questões existentes da fonte 2025
        cursor.execute("DELETE FROM questoes WHERE fonte = ?", ('Processo Seletivo 2025',))
        print("Removidas questões existentes da fonte: Processo Seletivo 2025")
        ...
        for filename in image_files:
            try:
                ...
                image_path = os.path.join("2025_questions_imgs", filename)
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    f"Questão {questao_num} - Processo Seletivo 2025",
                    question_data['a'],
                    question_data['b'],
                    question_data['c'],
                    question_data['d'],
...
def main():
    """Função principal para importar prova 2025 unificada"""
    success = import_questions_with_images_2025()
    
    if success:
        print("\n🎉 Importação da prova 2025 concluída com sucesso!")
        print("Cada questão agora possui sua imagem e alternativas corretamente associadas.")
    else:
        print("\n💥 Falha na importação.")
        print("\n📝 Para usar este script:")
        print("1. Crie o arquivo '2025_questions_alt.txt' com as questões e alternativas")
        print("2. Certifique-se de que a pasta '2025_questions_imgs' contém as imagens das questões")
