...
def create_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS questoes (
            ...
            e TEXT,
            gabarito TEXT,
            fonte TEXT DEFAULT 'Processo Seletivo 2022',
...
def extract_questions_from_txt_2022(txt_path="2022_questions_alt.txt"):
    """Extrai questões da prova 2022 do arquivo .txt"""
    ...
    questions = extract_questions_from_text_2022(text)
    return questions

def extract_questions_from_text_2022(text):
    """Extrai questões do texto da prova 2022"""
...
def import_questions_with_images_2022():
    """Importa questões com imagens e alternativas para 2022"""
    print("IMPORTADOR UNIFICADO - PROVA 2022")
    print("=" * 50)
    ...
    questions = extract_questions_from_txt_2022()
    if not questions:
    ...
    image_files = get_image_files_2022()
    if not image_files:
    ...
    try:
        ...
        # Limpar questões existentes da fonte 2022
        cursor.execute("DELETE FROM questoes WHERE fonte = ?", ('Processo Seletivo 2022',))
        print("Removidas questões existentes da fonte: Processo Seletivo 2022")
        ...
        for filename in image_files:
            try:
                ...
                image_path = os.path.join("2022_questions_imgs", filename)
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    f"Questão {questao_num} - Processo Seletivo 2022",
                    question_data['a'],
                    question_data['b'],
                    question_data['c'],
                    question_data['d'],
...
def main():
    """Função principal para importar prova 2022 unificada"""
    success = import_questions_with_images_2022()
    
    if success:
        print("\n🎉 Importação da prova 2022 concluída com sucesso!")
